...
async def admin_text(event):
    ...
    if m == "add_src":
        ...
        if cid not in CONFIG["bots"][b]["sources"]:
            ...
            QUEUES[b].setdefault(str(cid), [])
        save_config(CONFIG); STATE["mode"] = None
...
async def buttons(event):
    ...
    if d == "add_bot":
        STATE["mode"] = "add_bot"
        await event.edit("Send: @BotUsername bot_id", buttons=[[Button.inline("⬅ Back", b"back")]]); return
    ...
    if d == "rm_bot":
        ...
        await event.edit("❌ Bot removed", buttons=panel()); return

    if d in ("add_src", "rm_src", "add_dest", "rm_dest", "set_store"):
        STATE["mode"] = d
        await event.edit("Send input now", buttons=[[Button.inline("⬅ Back", b"back")]]); return
    ...
    if not b or b not in CONFIG["bots"]:
        await event.edit("❗ Please select a bot first", buttons=panel()); return
...
async def main():
    ...
    print("✅ SYSTEM RUNNING (FULL VERSION)")
